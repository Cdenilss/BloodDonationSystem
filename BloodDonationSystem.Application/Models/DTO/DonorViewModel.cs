...
{
    public DonorViewModel(Guid id, string name, string email, DateTime birthDate, GenderEnum gender, double weight,
        BloodTypeEnum bloodType, RhFactorEnum rhFactor, List<DonationViewModel> donations, AddressViewModel address)
    {
        Id = id;
        Name = name;
        Email = email;
    ...
    public static DonorViewModel FromEntity(Donor donor)
    {
        var donationViewModels = donor.Donations
            .Select(donation =>
                new DonationViewModel(donation.Id, donation.DateDonation, donation.QuantityMl,
                    null!)) // Passando null! para o DonorItemViewModel por enquanto
            .ToList();
